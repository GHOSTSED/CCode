...
void dlist_lock(DList *pList);
void dlist_unlock(DList *pList);
DList *dlist_init(void *mylock, fp_lock lockIt, fp_unlock unlockIt, fp_get_current_threadID getID);
DListNode *dlist_creat_node(void *data);
void dlist_delete_node(DListNode *pNode);
...
void dlist_lock(DList *pList)
{
	if (NULL != pList->listLocker)
	{
		locker_lockIt(pList->listLocker);
	}
}
...
void dlist_unlock(DList *pList)
{
	if (NULL != pList->listLocker)
	{
		locker_unlockIt(pList->listLocker);
	}
}
...
DList *dlist_init(void *mylock, fp_lock lockIt, fp_unlock unlockIt, fp_get_current_threadID getID)
{
	...
	if(NULL == mylock || NULL == lockIt || NULL == unlockIt || NULL == getID)
	{
		pLocker = NULL;
	}
	else
	{
		pLocker = locker_init(mylock, lockIt, unlockIt, getID);
		if(NULL == pLocker)
		{
			free(pTail);
			free(pHead);
...
void dlist_delete(DList *pList)
{
	...
	while (NULL != pList->first)
	{
		...
		dlist_delete_node(pNode);
	}
	if(NULL != pList->listLocker)
	{
		locker_delete(pList->listLocker);
	}
	free(pList);
}
